...
### And the show schedule, which was the other thing a capture was for

Every scene driver loops until `dbplayer.library` reports signal 1 — nothing in
the PowerPC code ever sets that flag, the 68K frame routine reads it from the
player. So the timeline lives in the music, and the music is generated by code
the harness already runs byte-exactly.

`work/re/showorder.py` closes it. `_play_part_1` contains 26 driver calls and
`_play_part_3` contains 13; walking the generated modules' order lists, **effect
7 parameter 1 occurs exactly 26 times and exactly 13 times** respectively. No
other effect/parameter pair matches either count, and the agreement holds on both
parts independently — a measurement, not a fit. Zipping the two gives every scene
its span:

| | scenes | driver calls | length |
|---|---|---|---|
| part one | 17 streams + 9 camera changes | 26 | 288.4 s |
| part three | 11 streams + 2 continuations | 13 | 149.9 s |

The tempo reading checks out against its own result: part three's boundaries land
on 15.000, 60.000, 75.000, 90.000 and 135.000 seconds exactly.

Two loose ends closed on the way. The four fog presets attach to named scenes in
table order, confirming the setter's call sites against the data. And the three
`new_camera` triples attach to three of the six scenes that record nothing on
their own — which is where the missing state for those is most likely to be.
...
- **FLAC is lossless.** Alignment is by audio cross-correlation, and this
  production's soundtrack can be regenerated byte-exactly by
  `work/re/runsynth.py`. So the correlation is between two clean signals rather
  than between a lossy re-encode and a guess — a better position than any other
  production in this repo started from.
- **640×480 matches the `__640`/`__480` constants** in the binary's float pool,
  which points to a native-resolution capture rather than a rescale.

It cannot be fetched from a cloud session — the host is not on the egress
allowlist and returns 403 (see `CLOUD_ENVIRONMENT.md`) — so fetching and pinning
is a local step. `tools/fetch/capture.mjs` now handles it directly.
