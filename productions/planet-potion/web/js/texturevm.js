...
// The two PowerPC floating-point semantics this VM is exact because of —
// truncating `stfs` stores and fused multiply-adds — are in fp.js, with the
// evidence that established each. Re-exported because callers of this module
// (work/re/texfloat.py's JS counterpart, the op handlers below) use them as
// part of the VM's surface.
// Imported AND re-exported, not `export … from`: that form creates no local
// binding, and the handlers below call both by name.
