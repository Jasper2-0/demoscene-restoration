...
// ---------------------------------------------------------------------------
export async function loadAtlas(d3d, url) {
  ...
  for (let i = 0; i < w * h; i++) {
    const b = rgba[i * 4 + 2];              // the original's `& 0xff`
    cov[i] = b;
    argb[i] = ((b << 24) | 0xffffff) >>> 0; // `buf[i] = buf[i] << 24 | 0xffffff`
  }
  const uv = buildGlyphTable(cov, w, h);
  // Levels 0 = a full mip chain, matching CreateTexture(..., Levels = 0, ...) at
  // the FUN_00403dd3 call site; the shim builds them with the original's own filter.
  const texture = d3d.createTexture(argb, w, h, { levels: 0 });
  return { texture, uv, width: w, height: h, coverage: cov };
}
