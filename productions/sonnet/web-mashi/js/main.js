...
// MASHI FORK: the pack ships ONE configuration — the REMASTER, exactly as the
// published site serves it.  Every URL knob below is pinned to its shipped
// value so esbuild folds the alternative branches out.  `?quality=original`,
// `?lighting=legacy`, `?assets=baked`, `?audio=party`, `?bg`, `?skip`,
// `?aniso`, `?prewarm`, `?warm`, `?warmstep`, `?flareburst` and `?debug` are
// verification and iteration controls; a delivery build answers none of them.
const AUTHENTIC = false;
const DEBUG = false;
// MASHI FORK.  `__HARNESS__` is a build-time boolean (tools/build_mashi.mjs):
// false for the shipped pack, true for the build tools/compare_mashi.mjs drives.
// With it false esbuild folds POS_PARAM to a literal null, which dead-codes the
// whole single-frame capture path at the bottom of this file — the largest lump
// of harness in the runtime — plus the asset prefetch and warm-store conditions
// that branch on it.  Keeping the hook available under a flag is what lets the
// fork be diffed against the untouched original frame-for-frame; see re/MASHI.md.
const POS_PARAM = __HARNESS__ ? params.get('pos') : null;
const PARTY = false;
const ASSET_MODE = 'generated';
...
const SKIP_PREWARM = false;
// The precalc disk cache — see web-mashi/js/warmstore.js for the equivalence
...
//
// `main.js` always sits at <root>/web-mashi/js/main.js, so `../../work/` is the work
...
// silently required the page to be at web-mashi/index.html; that assumption is
...
// MASHI FORK: the scene-jump table, its key bindings and the live-jump
// plumbing are removed.  1-9/0, [ ], and `,` `.` exist so a 7:41 demo can be
// inspected during development; a delivery build plays from the start.
// `?start=` went with them (it resolved names through the same table).
...
async function boot(assets, { wantAudio = false } = {}) {
  ...
  try {
    ...
    // ---- warm store (web-mashi/js/warmstore.js)
...
// MASHI FORK: `__HARNESS__ &&` is what actually dead-codes this branch.  Folding
// `const POS_PARAM = null` through the comparison is not something esbuild's
// constant inlining does here (measured: it kept the whole block), so the flag
// has to sit ON the condition, where it minifies to `if (false)`.
if (__HARNESS__ && POS_PARAM !== null) {
  ...
  overlay.addEventListener('click', async () => {
    ...
    // MASHI FORK: `jumpTo`, `window.__sonnetJump` and the `?start=` seek are
    // removed with the key bindings above — they were the only callers, and
    // the warm-to-position machinery they carried (the on-screen 'warming…'
    // note, the tag re-anchoring) has no purpose in a delivery build.
  }, { once: true });
}
