...
// The warm store's two seams (web-mashi/js/warmstore.js).  `textureObserver`
...
// MASHI FORK: lighting=fixed is pinned, so the bake is unconditional.
export const SHADOW_BAKE = true;
...
// MASHI FORK: the pack ships the REMASTER, so AUTHENTIC is a constant and the
// `?quality=original` arms (the missing dandelion stems, chiefly — a bug in the
// 2001 build that the remaster fixes) fold away.
export const AUTHENTIC = false;
export function setAuthentic() {}
