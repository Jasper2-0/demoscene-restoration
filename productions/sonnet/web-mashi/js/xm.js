...
export class XmPlayer {
  ...
  // MASHI FORK.  The volume column in this module carries ONLY a plain volume:
  // 5,598 cells in range 0x10..0x50 and not one volume-column EFFECT (the census
  // in re/MASHI.md scans every cell, so this is exhaustive).  So the slide,
  // fine-slide, vibrato-speed, vibrato, panning, pan-slide and tone-portamento
  // forms are all unreachable, and the per-tick half of the decoder does nothing
  // at all — `volumeColumnTick` is gone entirely and its call site with it.
  volumeColumnTickZero(c) {
    const vol = c.volColumn;
    if (vol >= 0x10 && vol <= 0x50) return (c.volume = c.outVolume = vol - 0x10);
    return vol;
  }
  ...
  rowEffect(c, fx, param, volColumnData = c.volColumn) {
    ...
    switch (fx) {
      // MASHI FORK — the module's effect census (re/MASHI.md) is exhaustive, not
      // sampled: all 43 patterns x 26 channels x 71,552 cells use only
      //   8  E9  4  Eb  9  3  Ec  A  6  1  2  5  7  Ea  E8  F
      // so every handler below that is gone was UNREACHABLE, not merely unused in
      // the part of the song a coverage run happened to reach.  Removed here:
      //   B  position jump      C  set volume        D  pattern break
      //   G  global volume      L  envelope position R  multi retrigger
      //   X  extra-fine porta   E1/E2 fine porta     E3 glissando
      //   E4/E7 waveform select E5 set finetune      E6 pattern loop
      //   ED note delay         EE pattern delay
      // plus the per-tick-only labels H, K, P and T.
      // Verified by tools/audio_ab.mjs: the whole 470 s song still renders
      // sample-for-sample identically to the untouched player.
      ...
      case 0xe:
        ...
        break;
      // These effects run on non-zero ticks; the row pass is a no-op for them.
      case 0x0: case 0x1: case 0x2: case 0x4: case 0x5: case 0x6:
      case 0x7: case 0x9: case 0xa:
        break;
      default:
  ...
  tickEffects() {
    for (const c of this.ch) {
      ...
      if (this.tick > 0 || this.repeatRow) {
        switch (c.effect) {
          // MASHI FORK: arpeggio (0), H, K, P, R and T are gone — see the census
          // note in rowEffect.  ED note delay goes with them, so `triggerDelayed`
          // and `c.delayed` are unreachable too.
          case 0x1: {
  ...
  tonePorta(c) {
    ...
    // MASHI FORK: `glissando` is set only by E3, which never appears in this
    // module, so the flag is permanently false and the note-quantising branch
    // (and `period2NotePeriod`, its last caller) are unreachable.
    c.outPeriod = c.period;
  }
  ...
  vibrato(c, param) {
    ...
    // MASHI FORK: the waveform selectors E4/E7 never appear, so vibWave and
    // tremWave are permanently 0 — the sine table is the only reachable arm,
    // and `resetInstrument`'s `& 4` continue-flag tests are always true.
    const wave = VIBRATO_TABLE[(c.vibPos >> 2) & 31];
  ...
  tremolo(c, param) {
    ...
    const wave = VIBRATO_TABLE[(c.tremPos >> 2) & 31];
