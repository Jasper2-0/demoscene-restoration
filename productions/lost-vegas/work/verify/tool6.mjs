...
await pg.evaluate(()=>{ document.getElementById('trace').click();
    const w=document.getElementById('bW'); w.value=String(+w.value+3); w.dispatchEvent(new Event('input'));
    const y=document.getElementById('bY'); y.value=String(+y.value-1); y.dispatchEvent(new Event('input')); });
