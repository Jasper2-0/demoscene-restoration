...
export function wonderEnvironmentTexcoords(normals, modelViewMatrix, scale = [1, 1, 1], output = null) {
  ...
  for (let vertex = 0; vertex < normals.length / 3; vertex++) {
    ...
    // 0x407547-0x4075d0 divides the complete first matrix row by twice
    // the sampled X scale and the complete second row by twice Y. It does
    // not normalize each basis column independently.
    const eyeX = (nx * m[0] + ny * m[4] + nz * m[8]) / (2 * scaleX);
    const eyeY = (nx * m[1] + ny * m[5] + nz * m[9]) / (2 * scaleY);
...
export class ExpSceneRenderer {
  constructor(mgl, scene, assets, { lighting = false, sphereMap = false } = {}) {
    ...
    this.geometries = scene.meshes.map((mesh) => buildMeshGeometry(mesh, {
      wonderNormals: scene.variant === 'wonder',
    }));
    ...
    selectedMeshes.forEach(({ sampledMesh, index }) => {
      ...
      if (environmentUnits?.size) {
        ...
        environmentTexcoords = wonderEnvironmentTexcoords(
          this.scene.variant === 'wonder' ? geometry.nativeNormals : geometry.normals,
